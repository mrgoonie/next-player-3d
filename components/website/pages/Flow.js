...
const pageSize = 10;
const apiPath = "/api/v1/flow";

export default function Flow({ user, res, query, params }) {
	const router = useRouter();
	const { case_slug } = router.query;
	// console.log(case_slug);
	const caseId = case_slug[0];
	...
	const loadData = (_page) => {
		...
		fetchApi({
			path: `${apiPath}?case=${caseId}&team=${session.user.team_id}&populate=owner&page=${_page}&size=${pageSize}`,
		}).then((res) => {
